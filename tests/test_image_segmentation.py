...
def test_produce():
    ...
    segmentation = ImageSegmentationPrimitive(
        hyperparams=segmentation_hp(segmentation_hp.defaults(), use_columns=[0]),
        volumes={"moco_weights": moco_path},
    )
...
def test_serialize(dataset="LL1_bigearth_landuse_detection", target_class="Pastures"):
    ...
    pipeline = ImageSegmentationPipeline(
        binary_labels, epochs_frozen=1, epochs_unfrozen=1
    )
