...
from kf_d3m_primitives.remote_sensing.image_retrieval.image_retrieval_pipeline import (
...
def iterative_labeling(features, labels, seed_idx=2, n_rounds=5):
    ...
    for i in range(n_rounds):
        ...
        results = {
            ...
            "n_neg": n_neg,
            "a_p": [
                float(y[inc_labeled[:k]].mean()) for k in 2 ** np.arange(11)
            ],  # precision, including labeled
            "u_p": [
                float(y[exc_labeled[:k]].mean()) for k in 2 ** np.arange(11)
            ],  # precision, excluding labeled
            "r_p": [
                float(y[inc_labeled[:k]].sum() / y.sum()) for k in 2 ** np.arange(11)
