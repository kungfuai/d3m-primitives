...
def read_data(dataset_name: str):
    train = pd.read_csv(f'../datasets/seed_datasets_current/{dataset_name}/TRAIN/dataset_TRAIN/tables/learningData.csv')
    test = pd.read_csv(f'../datasets/seed_datasets_current/{dataset_name}/SCORE/dataset_SCORE/tables/learningData.csv')
...
def train_test_dists(
    ...
    for (grp_name, test_f) in test_df.groupby(group_cols):
        ...
        num_0_obs.append(
            (train_f.iloc[:, target_cols[dataset_name]] == 0).sum().sum()
        )
        ...
        train_f.iloc[:, grouping_cols[dataset_name]] = \
            train_f.iloc[:, grouping_cols[dataset_name]].ffill()
        ...
        test_f.iloc[:, grouping_cols[dataset_name]] = \
            test_f.iloc[:, grouping_cols[dataset_name]].bfill()
    ...
    return series_lens, prop_obs, prop_0_obs, pred_lens, pd.concat(all_train_dfs), pd.concat(all_test_dfs)
...
def series_dists(train: pd.DataFrame, test: pd.DataFrame, dataset_name: str):
    ...
    series_lens, prop_obs, prop_0_obs, pred_lens, all_train_dfs, all_test_dfs = train_test_dists(
        train, 
        test,
        group_cols, 
        dataset_name
    ...
    plot_hist(prop_0_obs, 'Distribution of Proportion of 0 Values (of non-missing obserations)')
...
def grouping_dists(train: pd.DataFrame, dataset_name: str):
    ...
    for i in range(len(group_cols) - 1):
        ...
        plot_hist(num_series, f'Distribution of Training Series Lengths, Grouping Level: {i+1}')
...
def cluster_examples(
    ...
    max_l = max([
        len(trg) + len(teg)
        for (_, trg), (_, teg) 
        in zip(train_groups, test_groups)
    ])
    ...
    for i in range(nclusters):
        ...
        for j, ex in enumerate(examples):
            ...
            values = train.query(' & '.join(query_list)).iloc[:, target_cols[dataset_name]]
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--dataset-name', help='The name of the dataset to explore')
