...
class SlothTestCase(unittest.TestCase):
    ...
    _dataset_path = path.abspath(path.join(path.dirname(__file__), 'timeseries_dataset'))
    ...
    def test_defaults_produce(self) -> None:
        ...
        # mark grouping key
        dataframe.metadata = dataframe.metadata.add_semantic_type((metadata_base.ALL_ELEMENTS,1), 'https://metadata.datadrivendiscovery.org/types/GroupingKey')

        # create the clustering primitive and cluster
        hyperparams_class = Sloth.metadata.query()['primitive_code']['class_type_arguments']['Hyperparams']
        ...
        self.assertListEqual(list(column_metadata['semantic_types']),
    ...
    def test_defaults_produce_clusters(self) -> None:
        ...
        self.assertListEqual(result.iloc[:,0].tolist(), ['alpha', 'bravo', 'charlie', 'delta'])
